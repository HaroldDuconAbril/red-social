...
// Render (como muchos hostings) pone la app detrás de un proxy/balanceador,
// así que la IP real del cliente llega en el header X-Forwarded-For en vez de
// en la conexión TCP directa. Sin esto, Express usa la IP del proxy de Render
// para TODOS los usuarios, lo que rompe el rate limiting (y express-rate-limit
// directamente lo rechaza con ERR_ERL_UNEXPECTED_X_FORWARDED_FOR).
...
const verificationLimiter = rateLimit({
    ...
    // ipKeyGenerator() normaliza IPv6 (agrupa por subred /56) para que nadie
    // evada el límite simplemente rotando los últimos bits de su IPv6.
